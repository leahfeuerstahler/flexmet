...
FMP model can also transform the two-, three-, and four-parameter models.

# The FMP Model

The FMP IRF is specified using the composite function,

\[
P_i(\theta)=[1+\exp(-m(\theta))]^{-1},
\]
where for item $i$, $m_{i}(\theta)$ is an unbounded and monotonically
increasing polynomial function of the latent trait $\theta$. Specifically,
let 

\[
m_{i}(\theta)=b_{0i}+b_{1i}\theta+b_{2i}\theta^{2}+\cdots+
b_{2k_{i}+1,i}\theta^{2k_{i}+1},
\]
where $2k_{i}+1$ equals the order of the polynomial for item $i$,
$k_{i}$ is a nonnegative integer, and $\boldsymbol{b}_{i}=(b_{0i},b_{1i},\ldots,b_{2k_{i}+1,i})^{\prime}$
are item parameters that define the location and shape of the IRF.

Currently, flexmet only handles binary (0/1) item responses. Some functions 
also include a lower asymptote parameter $c_i$ and upper asymptote parameter
